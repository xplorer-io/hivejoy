            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {user ? (
                <div className="p-3 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg text-sm">
                  ...
                  <p className="text-xs text-green-600 dark:text-green-400 mt-1">User ID: {user.id}</p>
                </div>
              ) : (
                <div className="p-4 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-lg">
                  <p className="text-sm text-amber-800 dark:text-amber-200 font-medium mb-2">⚠️ Please sign in first</p>
                  <p className="text-xs text-amber-700 dark:text-amber-300 mb-3">
                    You need to be signed in to create a producer.
                  </p>
                  <Button type="button" variant="outline" onClick={() => { window.location.href = '/auth'; }} className="w-full">
                    Go to Sign In
                  </Button>
                </div>
              )}
            ...
            {success && supabaseData && (
              <div className="mt-6 p-4 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-green-800 dark:text-green-200">Success! Data saved to Supabase</h3>
                </div>
                <div className="space-y-2 text-sm">
                  ...
                  <code className="text-xs block break-all p-2 bg-background rounded">{supabaseData.profileImage}</code>
                  {supabaseData.profileImage && (
                    <div className="relative w-full h-48 border rounded-lg overflow-hidden bg-muted mt-4">
                      <Image src={supabaseData.profileImage} alt="From Supabase" fill className="object-contain" unoptimized />
                    </div>
                  )}
                </div>
                <Button onClick={handleReset} variant="outline" className="w-full">Create Another</Button>
              </div>
            )}
            {error && (
